...
class ArangoChair extends EventEmitter {
    ...
    _startLoggerState() {
        this.req.get({path:this._loggerStatePath}, (status, headers, body) => {
            ...
            const tidStartBuffer        = Buffer.from('tid":"');
            ...
            const ticktock = () => {
                ...
                this.req.get({path:`${this._loggerFollowPath}?from=${lastLogTick}`}, (status, headers, body) => {
                    console.log({path:`${this._loggerFollowPath}?from=${lastLogTick}`});
                    ...
                    start = idx = 0;
                    while(true) {
                        console.log("in _startLoggerState:ticktock:while");
                        idx = body.indexOf('\n', start);
                        
                        if (-1 === idx) break;

                        entry = body.slice(start, idx);
                        console.log("buffer from database");
                        ...
                        console.log({typeindexstart:idx0});
                        idx1 = entry.indexOf(commaDoubleTickBuffer, idx0);    // find ,"
                        console.log({typeindexend:idx1});
                        type = entry.slice(idx0, idx1).toString();

                        idx0 = entry.indexOf(tidStartBuffer) + 6;            // find tid":
                        console.log({tidindesstart:idx0});
                        idx1 = entry.indexOf(commaDoubleTickBuffer, idx0) - 1;    // find ,"
                        console.log({tidindexend:idx1});
                        tid = entry.slice(idx0, idx1).toString();
                        console.log({tid});
